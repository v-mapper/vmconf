...
newver="$(curl -s -k -L $(get_pd_user) -H "origin: $origin" "$pserver/mad_apk/vmapper/noarch")"
...
if checkupdate "$newver" "$installedver" ;then
 ...
 until curl -o /sdcard/Download/vmapper.apk -s -k -L $(get_pd_user) -H "origin: $origin" "$pserver/mad_apk/vmapper/download" ;do
  rm -f /sdcard/Download/vmapper.apk
  sleep
 done
 /system/bin/pm install -r /sdcard/Download/vmapper.apk
